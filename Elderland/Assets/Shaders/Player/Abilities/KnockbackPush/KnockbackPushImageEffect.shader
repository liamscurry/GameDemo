...
// If there is an issue where the grab texture is not clearing, make sure to set the material
// to be in the transparent (3000) render queue in the inspector.
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i, float3 normal : NORMAL) : SV_Target
            {
                ...
                float nearPlaneCameraStrength = saturate(-i.cameraPos.z - _ProjectionParams.y * 10);
                ...
                float2 centerDisplacement = i.uvOriginal - float2(0.5, 0.5);
                ...
                float4 existingColor = tex2Dproj(_WarpTexture, i.grabPos + warpDirection);
